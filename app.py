...
if "file_uploader_reset" not in st.session_state:
    st.session_state.file_uploader_reset = 0

# --- SIDEBAR: SINGLE UPLOAD SECTION ---
with st.sidebar:
    st.header("Attachments")
    uploaded_file = st.file_uploader(
        "Upload any file (PDF, Image, Word, TXT, CSV, Excel) Max size: 2MB",
        type=["pdf", "jpg", "png", "jpeg", "docx", "txt", "csv", "xlsx"],
        key=f"uploader_{st.session_state.file_uploader_reset}",
    ...
    if uploaded_file:
        ...
        if file_data:
            ...
            st.error(
                "Unsupported file type. Please upload PDF, Image, Word, TXT, CSV, or Excel."
            )
...
if file_data:
    ...
    if st.session_state.generated_questions is None:
        st.session_state.generated_questions = generate_file_questions(
    ...
    if not st.session_state.questions_asked:
        questions_text = "\n".join(
            [f"{i+1}. {q}" for i, q in enumerate(st.session_state.generated_questions)]
        )
    ...
    if prompt := st.chat_input("Ask your own question about the file..."):
        try:
            ...
            if "context_length_exceeded" in error_msg.lower() or "400" in error_msg:
                st.error(
                    "File too large. Try a smaller file or more specific question."
                )
    ...
    st.warning(
        "Upload a file (PDF, Image, Word, TXT, CSV, Excel) to get started. Max size: 2MB"
    )
